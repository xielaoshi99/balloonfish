<template>
  <div>
    ...
    <el-table size="mini" :data="surperTableData" border max-height="585" style="width: 100%">
      <el-table-column fixed v-if="surperTableLabel[0]" :prop="surperTableLabel[0]" :label="surperTableLabel[0]" width="250"></el-table-column>
      <el-table-column v-for="(data, index) in surperTableLabel.slice(1)" :key="index" :prop="data" :label="data" width="180"></el-table-column>
    </el-table>
    <!-- 超级表分页 -->
    ...
    methods: {
      ...
      getSTableData(isFirst, isResetPage) {
        ...
        if (this.surperTableFilter.surperDateRange) {
          console.log(this.surperTableFilter.surperDateRange)
        ...
        if (this.tablename) {
          selectData(this.tablename, this.dbname, this.link, this.surperTableFilter.fields, this.surperWhere, this.eachPageSurperTable, offsetVal, this.surperTorder, startTime, endTime).then((data) => {
            if (data.res) {
              //成功
